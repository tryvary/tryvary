<?php
/**
 * WP Bootstrap Navwalker - TryVary
 *
 * @package tryvary
 *
 */
...
if ( ! class_exists( 'Tryvary_Bootstrap_Navwalker' ) ) :
	/**
	 * Tryvary_Bootstrap_Navwalker class.
	 */
	class Tryvary_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			 ...
			 * @param int      $depth   Depth of menu item. Used for padding.
			 */
			$class_names = join( ' ', apply_filters( 'tryvary_nav_menu_submenu_css_class', $classes, $args, $depth ) );
		 ...
		 *
		 * @since WP 3.0.0
		 * @since WP 4.4.0 The {@see 'tryvary_nav_menu_item_args'} filter was added.
		 *
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			 ...
			 */
			$args = apply_filters( 'tryvary_nav_menu_item_args', $args, $item, $depth );
			...
			$classes = apply_filters( 'tryvary_nav_menu_css_class', array_filter( $classes ), $item, $args, $depth );
			 ...
			 * @param int              $depth   Depth of menu item. Used for padding.
			 */
			$id = apply_filters( 'tryvary_nav_menu_item_id', 'menu-item-' . $item->ID, $item, $args, $depth );
			...
			$atts = apply_filters( 'tryvary_nav_menu_link_attributes', $atts, $item, $args, $depth );
			...
			$title = apply_filters( 'tryvary_the_title', $item->title, $item->ID );
			 ...
			 * @param int              $depth Depth of menu item. Used for padding.
			 */
			$title = apply_filters( 'tryvary_nav_menu_item_title', $title, $item, $args, $depth );
			...
			$output .= apply_filters( 'tryvary_walker_nav_menu_start_el', $item_output, $item, $depth, $args );
		}
		...
		public static function fallback( $args ) {
			...
			if ( $args['container'] ) {
				 ...
				 */
				$allowed_tags = apply_filters( 'tryvary_wp_nav_menu_container_allowedtags', array( 'div', 'nav' ) );
			...
			$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu', 'tryvary' ) . '">' . esc_html__( 'Add a menu', 'tryvary' ) . '</a></li>';
			$fallback_output .= '</ul>';
